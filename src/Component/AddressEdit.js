...
//                   />
//                   <button
//                     className="bg-purple-500 p-1 w-20 rounded-2xl text-white"
//                     type="submit"
//                   >
//                     Update
//                   </button>
...
export default function AddressEdit() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (loading) return;
    setLoading(true); // Set loading before dispatch
    dispatch(editAddress(credential, userInfo))
      .then(() => {
        setTimeout(() => {
          navigate(`/cart/${token}`);
      ...
      {loading ? (
        ...
        <Base>
          <div className="flex justify-center h-full">
            <div className="border shadow-xl shadow-gray-500 border-black pt-40 mt-20 md:mt-0 h-full w-5/6 md:h-[78%] lg:h-[78%] xl:h-[78%] bg-white text-center py-14 lg:py-20 xl:py-10 md:py-20 rounded-[50px] overflow-hidden">
              <div className="h-full md:overflow-y-auto hide-scrollbar">
